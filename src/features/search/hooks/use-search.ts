...
export function useGlobalSearch(
	...
	const paginationIsAvailable =
		pagination.isAvailable && queryHook.query.trim().length > 0;
	const isSearchPage = location.pathname === '/search';
	const hasPendingChanges =
		executionHook.loading ||
		isLoading ||
		queryHook.isDebouncing ||
		cuisineHook.isDebouncing;
	const emptyInputs =
		queryHook.query.trim().length === 0 &&
		cuisineHook.cuisineString.trim().length === 0;
	...
	useEffect(() => {
		const hasSearchInput =
			queryHook.debouncedQuery.trim().length > 0 ||
			cuisineHook.cuisineString.trim().length > 0;
		const isStable = !queryHook.isDebouncing && !cuisineHook.isDebouncing;

		const shouldAutomaticallySearch =
			isSearchPage && hasSearchInput && isStable;
		...
		queryHook.debouncedQuery,
		cuisineHook.cuisineString,
		queryHook.isDebouncing,
		cuisineHook.isDebouncing,
	]);
	...
	const executeSearch = async (page: number = pagination.activePage) => {
		...
		const sameQuery =
			lastSearch &&
			lastSearch.query === params.query &&
			lastSearch.cuisines === params.cuisines;
